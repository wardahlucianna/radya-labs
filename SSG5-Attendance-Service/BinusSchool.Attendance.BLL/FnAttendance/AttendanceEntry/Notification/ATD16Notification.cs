...
{
    public class ATD16Notification : FunctionsNotificationHandler, IDisposable
    {
        private IDictionary<string, object> _notificationData;
        private IReadOnlyCollection<ParentAttendanceVm<LateDayAttendanceVm>> _parents;
        private readonly IAttendanceDbContext _dbContext;
        private readonly ILogger<ATD16Notification> _logger;

        public ATD16Notification(
            INotificationManager notificationManager,
            DbContextOptions<AttendanceDbContext> options,
            ILogger<ATD16Notification> logger,
            IConfiguration configuration,
            IDictionary<string, object> notificationData) :
            base("ATD16", notificationManager, configuration, logger)
        {
            ...
            try
            {
                ...
                var schedules = await _dbContext.Entity<TrGeneratedScheduleLesson>()
                                                ...
                                                .Include(x => x.GeneratedScheduleStudent)
                                                 ...
                                                 .Where(x => listIdScheduleLessonEmail.Contains(x.Id))
                                                 .ToListAsync(CancellationToken);

                if (!schedules.Any())
                    return;

                var parents = new List<ParentAttendanceVm<LateDayAttendanceVm>>();
                ...
                foreach (var student in students)
                {
                    ...
                    {
                        parents.Add(new ParentAttendanceVm<LateDayAttendanceVm>
                        {
                            IdUser = parent.Parent.Id,
                            EmailAddress = parent.Parent.PersonalEmailAddress,
                            FirstName = parent.Parent.FirstName,
                            LastName = parent.Parent.LastName,
                            StudentAttendance = new LateDayAttendanceVm
                            {
