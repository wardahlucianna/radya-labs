...
{
    public class GetSummaryByRangeHandler : FunctionsHttpSingleHandler
    {
        ...
        public GetSummaryByRangeHandler(
        ...
        protected async override Task<ApiErrorResult<object>> Handler()
        {
            ...
            if (param.SelectedPosition == PositionConstant.SubjectTeacher)
            {
               ...
               .ToListAsync(CancellationToken);
                ...
                var idLevelAllowed = _dbContext.Entity<MsMappingAttendance>().Where(x => _idLevel.Contains(x.IdLevel) && x.AbsentTerms == AbsentTerm.Session).Select(x => x.IdLevel).ToList();
            ...
            if (param.SelectedPosition == PositionConstant.SubjectHead || param.SelectedPosition == PositionConstant.SubjectHeadAssitant)
            {
                var idLevelAllowed = _dbContext.Entity<MsMappingAttendance>().Where(x => idLevelForSHAndSha.Contains(x.IdLevel) && x.AbsentTerms == AbsentTerm.Session).Select(x => x.IdLevel).ToList();
                predicateLesson = predicateLesson.And(x => idLevelAllowed.Contains(x.Homeroom.Grade.IdLevel));
            }
            ...
            var idLevels = allSchedules.Select(x => x.Homeroom.Grade.IdLevel).Distinct().ToList();
            ...
            foreach (var item in result)
            {
                ...
                .Where(x => x.IdAcademicYear == param.IdAcademicYear && (x.StartDate == param.StartDate.Date || x.EndDate == param.EndDate.Date 
                ...
                {
                    if(studentId.Any())
                        allSchedules = allSchedules.Where(x => !studentId.ToList().Contains(x.GeneratedScheduleStudent.IdStudent)).ToList();
                }

                var schedules = allSchedules.Where(x => homeroomIds.Contains(x.IdHomeroom)).ToList();
                var schedulessubmittedList = allSchedules.Where(x => homeroomIds.Contains(x.IdHomeroom) && x.AttendanceEntries.Count != 0).ToList();
                var schedulesUnsubmittedList = allSchedules.Where(x => homeroomIds.Contains(x.IdHomeroom) && x.AttendanceEntries.Count == 0).ToList();
                ...
                foreach (var dataUnsubmitted in schedulesUnsubmittedList)
                {
                    ...
                    {
                        if (schedulessubmittedList.Any(x => x.ScheduleDate == dataUnsubmitted.ScheduleDate && x.IdHomeroom == dataUnsubmitted.IdHomeroom  &&
                ...
                item.TotalStudent = schedules.GroupBy(x => x.GeneratedScheduleStudent.IdStudent).Count();
                item.Submitted = mappingLevel.AbsentTerms == AbsentTerm.Session ?
                                schedules
                                        .Where(x => x.AttendanceEntries.Any(y => y.Status == AttendanceEntryStatus.Submitted))
                                        .GroupBy(x => new { x.ScheduleDate, x.IdSession }).Count() :
                                 schedules.Where(x => x.AttendanceEntries.Any(y => y.Status == AttendanceEntryStatus.Submitted)).GroupBy(x => new { x.ScheduleDate }).Count();
                item.Pending = mappingLevel.AbsentTerms == AbsentTerm.Session ?
                               schedules.Where(x => x.AttendanceEntries.Any(y => y.Status == AttendanceEntryStatus.Pending))
                ...
                item.Unsubmitted = mappingLevel.AbsentTerms == AbsentTerm.Session ?
                                   checkStudentStatus == null ? schedulesUnsubmitted.Where(x => !x.AttendanceEntries.Any()
                                                        && x.ScheduleDate <= _dateTime.ServerTime.Date)
                                            //.GroupBy(x => new { x.ScheduleDate, x.ClassID, x.IdSession })
                                            .GroupBy(x => new { x.ScheduleDate, x.ClassID, x.IdSubject, x.SubjectName, x.IdHomeroom, x.HomeroomName, x.IdSession, x.SessionID })
                                            .Count() : schedulesUnsubmitted.Where(x => !x.AttendanceEntries.Any()
                                                        && !checkStudentStatus.Select(y => y.IdStudent).ToList().Contains(x.GeneratedScheduleStudent.IdStudent)
                                                        && x.ScheduleDate <= _dateTime.ServerTime.Date)
                                            .GroupBy(x => new { x.ScheduleDate, x.ClassID, x.IdSubject, x.SubjectName, x.IdHomeroom, x.HomeroomName, x.IdSession, x.SessionID })
                                            .Count():
                                   checkStudentStatus == null ? schedulesUnsubmitted.Where(x => !x.AttendanceEntries.Any()
            ...
            Logger.LogInformation("By range old takes {Seconds}", Math.Round(sw.Elapsed.TotalSeconds, 2));
