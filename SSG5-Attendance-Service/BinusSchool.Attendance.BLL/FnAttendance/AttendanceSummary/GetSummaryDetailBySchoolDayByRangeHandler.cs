...
{
    public class GetSummaryDetailBySchoolDayByRangeHandler : FunctionsHttpSingleHandler
    {
        private readonly IAttendanceDbContext _dbContext;
        private readonly IMachineDateTime _dateTime;
        public GetSummaryDetailBySchoolDayByRangeHandler(
        ...
        private List<Session> GetSessions(MsMappingAttendance mapping, IEnumerable<TrGeneratedScheduleLesson> data)
        {
            ...
            if (mapping.AbsentTerms == AbsentTerm.Session)
            {
                 ...
                 .Select(x => new Session
                 {
                     ...
                     Attendances = mapping.AttendanceMappingAttendances.Select(y => new AttendanceData
                     {
                         ...
                         Students = x.Where(z => z.AttendanceEntries.Any(a => a.AttendanceMappingAttendance.IdAttendance == y.IdAttendance && a.Status == AttendanceEntryStatus.Submitted))
                                     .GroupBy(z => new { idStudent = z.GeneratedScheduleStudent.Student.Id, studentName = $"{z.GeneratedScheduleStudent.Student.FirstName} {z.GeneratedScheduleStudent.Student.LastName}" })
                                     .Select(z => new ItemValueVm
                                     {
                                         Id = z.Key.idStudent,
                                         Description = z.Key.studentName
                                     }).ToList()
                     }).ToList(),
                ...
                {
                    var listIdStudentSubmitted = item.Attendances.SelectMany(x => x.Students.Select(y => y.Id).ToList()).ToList();
                    item.Unsubmitted = item.Unsubmitted.Where(x => !listIdStudentSubmitted.Contains(x.Id)).Distinct().ToList();
                }
                return result;
            }
            else
            {
                var result = data.GroupBy(x => new { x.ScheduleDate, x.IdHomeroom, x.HomeroomName })
                                     .Select(x => new Session
                                     {
                                         Id = x.Key.ScheduleDate.ToString(),
                                         Description = x.Key.HomeroomName,
                                         ClassId = x.Key.IdHomeroom,
                                         Attendances = mapping.AttendanceMappingAttendances.Select(y => new AttendanceData
                                         {
                                             Id = y.Id,
                                             Code = y.Attendance.Code,
