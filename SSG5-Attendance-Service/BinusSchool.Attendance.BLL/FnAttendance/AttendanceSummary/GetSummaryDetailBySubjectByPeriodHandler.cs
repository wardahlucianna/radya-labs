...
{
    public class GetSummaryDetailBySubjectByPeriodHandler : FunctionsHttpSingleHandler
    {
        private static readonly Lazy<string[]> _requiredParams = new Lazy<string[]>(new[]
        {
            nameof(GetSummaryDetailByPeriodRequest.IdAcademicYear),
        ...
        public GetSummaryDetailBySubjectByPeriodHandler(
        ...
        {
            var param = Request.ValidateParams<GetSummaryDetailByPeriodRequest>(_requiredParams.Value);
            
            #region Get Position User
            
            var positions = await _positionByUserHandler.GetAvailablePositionDetail(param.IdUser, param.IdAcademicYear);
            ...
            if (selectedOtherPositions.Length != 0)
            {
                if (param.SelectedPosition == PositionConstant.Principal)
                {
                    ...
                    predicateLevelPrincipalAndVicePrincipal = predicateLevelPrincipalAndVicePrincipal
                        .And(x => idLevelPrincipalAndVicePrincipal.Contains(x.Grade.IdLevel));
                }
            ...
            else if (param.SelectedPosition == PositionConstant.SubjectTeacher && positions.SubjectTeachers.Count != 0)
            {
                ...
                var idLessons = positions.ClassAdvisors.Select(x => x.Id).ToArray();
            ...
            result.Data = schedules.GroupBy(x => new { x.IdHomeroom, x.HomeroomName, x.ClassID, x.IdSubject, x.SubjectName, x.IdUser, x.TeacherName })
