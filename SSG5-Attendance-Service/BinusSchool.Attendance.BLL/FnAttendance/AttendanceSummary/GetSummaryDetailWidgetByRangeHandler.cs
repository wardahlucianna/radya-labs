...
{
    public class GetSummaryDetailWidgetByRangeHandler : FunctionsHttpSingleHandler
    {
        private readonly IAttendanceDbContext _dbContext;
        private readonly IMachineDateTime _dateTime;
        public GetSummaryDetailWidgetByRangeHandler(
        ...
        protected async override Task<ApiErrorResult<object>> Handler()
        {
            ...
            return Request.CreateApiResult2(new SummaryDetailWidgetResult
            {
                ...
                Present = mappingLevel.AbsentTerms == AbsentTerm.Day ?
                          ...
                          schedules.Where(x => x.AttendanceEntries.Any(y => y.AttendanceMappingAttendance.Attendance.AttendanceCategory == AttendanceCategory.Present && y.AttendanceMappingAttendance.Attendance.Code != "LT" && y.AttendanceMappingAttendance.Attendance.AbsenceCategory == null && y.Status == AttendanceEntryStatus.Submitted)).GroupBy(x => new { x.SessionID, x.SubjectName, x.IdHomeroom, x.ScheduleDate, x.GeneratedScheduleStudent.IdStudent }).Count(),
                Late = mappingLevel.AbsentTerms == AbsentTerm.Day ?
                       ...
                       schedules.Where(x => x.AttendanceEntries.Any(y => y.AttendanceMappingAttendance.Attendance.Code == "LT" && y.Status == AttendanceEntryStatus.Submitted)).GroupBy(x => new { x.SessionID, x.SubjectName, x.IdHomeroom, x.ScheduleDate, x.GeneratedScheduleStudent.IdStudent }).Count(),
                ExcusedAbsence = mappingLevel.AbsentTerms == AbsentTerm.Day ?
                                 ...
                                 schedules.Where(x => x.AttendanceEntries.Any(y => y.AttendanceMappingAttendance.Attendance.AbsenceCategory == AbsenceCategory.Excused && y.Status == AttendanceEntryStatus.Submitted)).GroupBy(x => new { x.SessionID, x.SubjectName, x.IdHomeroom, x.ScheduleDate, x.GeneratedScheduleStudent.IdStudent }).Count(),
                ...
                UnexcusedAbsence = mappingLevel.AbsentTerms == AbsentTerm.Day
                                   ? schedules.Where(x => x.AttendanceEntries.Any(y => y.AttendanceMappingAttendance.Attendance.AbsenceCategory == AbsenceCategory.Unexcused && y.Status == AttendanceEntryStatus.Submitted)).GroupBy(x => new { x.ScheduleDate, x.GeneratedScheduleStudent.IdStudent }).Count()
                                   : schedules.Where(x => x.AttendanceEntries.Any(y => y.AttendanceMappingAttendance.Attendance.AbsenceCategory == AbsenceCategory.Unexcused && y.Status == AttendanceEntryStatus.Submitted)).GroupBy(x => new { x.SessionID, x.SubjectName, x.IdHomeroom, x.ScheduleDate, x.GeneratedScheduleStudent.IdStudent }).Count()
            } as object);
        }
    }
}
