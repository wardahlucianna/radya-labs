...
{
    public class GetSummaryUnsubmittedByPeriodHandler : FunctionsHttpSingleHandler
    {
        private readonly IAttendanceDbContext _dbContext;
        private readonly IMachineDateTime _dateTime;
        public GetSummaryUnsubmittedByPeriodHandler(
        ...
        {
            var param = Request.ValidateParams<GetSummaryUnsubmittedByPeriodRequest>(nameof(GetSummaryUnsubmittedByPeriodRequest.IdAcademicYear),
                                                                         nameof(GetSummaryUnsubmittedByPeriodRequest.IdLevel),
                                                                         nameof(GetSummaryUnsubmittedByPeriodRequest.Semester),
                                                                         nameof(GetSummaryUnsubmittedByPeriodRequest.IdSchool),
                                                                         nameof(GetSummaryUnsubmittedByPeriodRequest.IdUser));
            ...
            var query = _dbContext.Entity<TrGeneratedScheduleLesson>()
                                        ...
                                        //         .ThenInclude(x => x.StudentGrades)
                                           ...
                                           .GroupBy(x => new { x.ScheduleDate, x.ClassID, x.IdUser, x.TeacherName, x.IdSubject, x.SubjectName, x.IdHomeroom, x.HomeroomName, x.IdSession, x.SessionID });
            ...
            if (param.OrderType == OrderType.Asc)
            {
                ...
                {
                    case "date":
                        query = query.OrderBy(x => x.Key.ScheduleDate);
                        break;
                    case "classId":
                        query = query.OrderBy(x => x.Key.ClassID);
                        break;
                    case "teacher":
                        query = query.OrderBy(x => x.Key.TeacherName);
                        break;
                    case "subject":
                        query = query.OrderBy(x => x.Key.ScheduleDate);
                        break;
                    case "homeroom":
                        ...
                        break;
                    case "session":
                        query = query.OrderBy(x => x.Key.SessionID);
                        break;
                    default:
                        query = query.OrderBy(x => x.Key.ScheduleDate);
                        break;
            ...
            else
            {
                ...
                {
                    case "date":
                        query = query.OrderByDescending(x => x.Key.ScheduleDate);
                        break;
                    case "classId":
                        query = query.OrderByDescending(x => x.Key.ClassID);
                        break;
                    case "teacher":
                        query = query.OrderByDescending(x => x.Key.TeacherName);
                        break;
                    case "subject":
                        query = query.OrderByDescending(x => x.Key.ScheduleDate);
                        break;
                    case "homeroom":
                        ...
                        break;
                    case "session":
                        query = query.OrderByDescending(x => x.Key.SessionID);
                        break;
                    default:
                        query = query.OrderByDescending(x => x.Key.ScheduleDate);
                        break;
            ...
            ? res.Count
            : await query.Select(x => x.Key.ScheduleDate).CountAsync(CancellationToken);
