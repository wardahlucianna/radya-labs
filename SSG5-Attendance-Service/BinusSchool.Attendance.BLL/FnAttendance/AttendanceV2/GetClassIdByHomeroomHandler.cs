...
{
    public class GetClassIdByHomeroomHandler : FunctionsHttpSingleHandler
    {
        private readonly IAttendanceDbContext _dbContext;
        private readonly IMachineDateTime _datetimeNow;

        public GetClassIdByHomeroomHandler(IAttendanceDbContext dbContext, IMachineDateTime datetimeNow)
        {
        ...
        protected override async Task<ApiErrorResult<object>> Handler()
        {
            ...
            var listIdLessonByLessonTeacher = await _dbContext.Entity<MsLessonPathway>()
                                                 .Where(e => e.HomeroomPathway.Homeroom.HomeroomTeachers.Any(f=>f.IdBinusian==param.IdUser)
                                                         && e.Lesson.IdAcademicYear == param.IdAcademicYear
                                                         && e.HomeroomPathway.IdHomeroom==param.IdHomeroom 
                                                         || listIdLessonRealization.Contains(e.IdLesson))
                                                 .Select(e => e.IdLesson)
                                                 .ToListAsync(CancellationToken);
            ...
            var listSchedule = await _dbContext.Entity<MsSchedule>()
                          ...
                          .Where(e => e.Lesson.IdAcademicYear == param.IdAcademicYear
                                  && e.Lesson.LessonPathways.Any(f=>f.HomeroomPathway.IdHomeroom==param.IdHomeroom)
            ...
            listScheduleLesoonData = listScheduleLesoonData.Where(x => listSchedule.Any(y => y.IdLesson == x.IdLesson && y.IdDay == x.IdDay && y.StartTime == x.StartTime && y.EndTime == x.EndTime)).ToList();
            ...
            List<GetClassIdByHomeroomResult> result = new List<GetClassIdByHomeroomResult>();
            for (var i = 0; i < listSchedule.Count(); i++)
            {
                ...
                if (session != null)
                {
                    ...
                    #region Submitted
                    var listAttendanceEntrySubmitted = listAttendanceEntry
                                                           .Where(e =>
                    ...
                    var submitted = listAttendanceEntrySubmitted.Count();
                    #endregion

                    #region pending
                    var listAttendanceEntryPending = listAttendanceEntry
                                                           .Where(e =>
                                                               listIdScheduleLesoon.Contains(e.IdScheduleLesson) &&
                    ...
                    var pending = listAttendanceEntryPending.Count();
                    #endregion

                    result.Add(new GetClassIdByHomeroomResult
                    {
                        ClassId = session.ClassID,
                        Session = new GetClassIdSession
                        {
