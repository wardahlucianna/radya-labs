...
{
    public class ATD2V2Notification : FunctionsNotificationHandler, IDisposable
    {
        private IDictionary<string, object> _notificationData;
        private IReadOnlyCollection<GetNotificationATD2Result> _getNotificationATDs;
        private readonly IAttendanceDbContext _dbContext;
        private readonly ILogger<ATD2V2Notification> _logger;

        public ATD2V2Notification(INotificationManager notificationManager,
             DbContextOptions<AttendanceDbContext> options,
             IConfiguration configuration,
             ILogger<ATD2V2Notification> logger,
             IDictionary<string, object> notificationData) :
              base("ATD2", notificationManager, configuration, logger)
        {
        ...
        private class GetNotificationATD2Result
        {
        ...
        protected override async Task Prepare()
        {
            try
            {
                ...
                var dataEntry =
                    (
                        from _attendanceEntry in _dbContext.Entity<TrAttendanceEntryV2>()
                        ...
                        join _scheduleLesson in _dbContext.Entity<MsScheduleLesson>() on _attendanceEntry.IdScheduleLesson equals _scheduleLesson.Id
                        join _session in _dbContext.Entity<MsSession>() on _scheduleLesson.IdSession equals _session.Id
                        join _homeroomStudent in _dbContext.Entity<MsHomeroomStudent>() on _attendanceEntry.IdHomeroomStudent equals _homeroomStudent.Id
                        ...
                        join _staffInput in _dbContext.Entity<MsStaff>() on _homeroomTeacher.IdBinusian equals _staffInput.IdBinusian
                        where
                        1 == 1
                        && listIdScheduleLessonEmail.Contains(_attendanceEntry.IdScheduleLesson)
                        && _homeroomTeacher.IsAttendance
                        && _attendance.AttendanceCategory == AttendanceCategory.Absent
                        select new GetNotificationATD2Result
                        {
                            ParentId = _parent.Id,
        ...
        protected override async Task SendPushNotification()
        {
            ...
            foreach (var item in _getNotificationATDs)
            {
                ...
                _notificationData["homeroom"] = item.Homeroom;
                _notificationData["binussianID"] = item.StudentId;
                _notificationData["attendanceStatus"] = item.Status.ToString();
                _notificationData["idSession"] = item.SessionId;
                _notificationData["emailReceiver"] = item.ParentEmail;
