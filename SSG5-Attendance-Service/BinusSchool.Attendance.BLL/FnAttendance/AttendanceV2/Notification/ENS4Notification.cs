...
{
    public class ENS4Notification : FunctionsNotificationHandler
    {
        private IDictionary<string, object> _notificationData;
        private readonly ILogger<ENS4Notification> _logger;
        ...
        private List<ENS4NotificationRequest> listAttendanceEmail =  new List<ENS4NotificationRequest>();
        public ENS4Notification(INotificationManager notificationManager, IConfiguration configuration, ILogger<ENS4Notification> logger, IAttendanceDbContext dbContext, IMachineDateTime datetime, IDictionary<string, object> notificationData, IAttendanceV2 attendanceService, IRolePosition rolePositionService) :
           base("ENS4", notificationManager, configuration, logger)
        {
        ...
        protected override async Task Prepare()
        {
            try
            {
                ...
                var listIdUser = await _dbContext.Entity<MsLessonTeacher>()
                                            .Include(e => e.Staff)
                                            .Where(e => e.Lesson.IdAcademicYear == idAcademicYear
                                                        && e.IsAttendance
                                                        && e.Lesson.Grade.Level.MappingAttendances.Any(f => f.AbsentTerms == AbsentTerm.Session))
                                            .Select(e => e.Staff)
                                            .Distinct()
                                            .ToListAsync(CancellationToken);

                List<Ens4NotificationUnsubmited> listUnsubmited = new List<Ens4NotificationUnsubmited>();
                ...
                foreach (var itemStaff in listIdUser)
                {
                    ...
                    if (GetUnsubmittedAttendanceV2.Attendances != null)
                    {
                        ...
                        listUnsubmited.AddRange(newUnsubmittedAttendance);
                    }
                }
                ...
                foreach (var idUser in listIdUserByPosition)
                {
                    ...
                    if (GetSubjectByUser != null)
                    {
                        ...
                        var listUnsubmitedByUserPosition = listUnsubmited
                                                            .GroupBy(e => new
                                                            {
                                                                e.ClassId,
                                                                e.Datetime,
                        ...
                        if (listUnsubmitedByUserPosition.Any())
                            listAttendanceEmail.Add(new ENS4NotificationRequest
                            {
                                IdUser = idUser,
