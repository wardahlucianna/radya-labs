...
{
    public class ATD18Notification : FunctionsNotificationHandler, IDisposable
    {
        private IDictionary<string, object> _notificationData;
        private IReadOnlyCollection<ParentAttendanceVm<AbsentDayAttendanceVm>> _parents;
        private readonly IAttendanceDbContext _dbContext;
        private readonly ILogger<ATD18Notification> _logger;

        public ATD18Notification(
            INotificationManager notificationManager,
            DbContextOptions<AttendanceDbContext> options,
            ILogger<ATD18Notification> logger,
            IConfiguration configuration,
            IDictionary<string, object> notificationData) :
            base("ATD18", notificationManager, configuration, logger)
        {
        ...
        protected override async Task Prepare()
        {
            try
            {
                ...
                var parents = new List<ParentAttendanceVm<AbsentDayAttendanceVm>>();
                ...
                foreach (var student in students)
                {
                    ...
                    {
                        parents.Add(new ParentAttendanceVm<AbsentDayAttendanceVm>
                        {
                            IdUser = parent.Parent.Id,
                            EmailAddress = parent.Parent.PersonalEmailAddress,
                            FirstName = parent.Parent.FirstName,
                            LastName = parent.Parent.LastName,
                            StudentAttendance = new AbsentDayAttendanceVm
                            {
