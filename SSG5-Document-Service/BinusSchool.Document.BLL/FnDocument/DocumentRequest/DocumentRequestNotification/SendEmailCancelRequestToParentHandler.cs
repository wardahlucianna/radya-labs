...
{
    public class SendEmailCancelRequestToParentHandler : FunctionsHttpSingleHandler
    {
        // MsNotificationTemplate - DB SCHOOL
        private readonly string _notificationScenario = "DRE02P";
        ...
        public SendEmailCancelRequestToParentHandler(
        ...
        {
            var param = await Request.ValidateBody<SendEmailCancelRequestToParentRequest, SendEmailCancelRequestToParentValidator>();

            var result = await SendEmailCancelRequestToParent(param);

            return Request.CreateApiResult2(result as object);
        }

        public async Task<SendEmailCancelRequestToParentResult> SendEmailCancelRequestToParent(SendEmailCancelRequestToParentRequest param)
        {
            ...
            // get Approver email
            var approverDataEmail = await _dbContext.Entity<MsDocumentReqApprover>()
                                        .Include(x => x.Staff)
                                        .Where(x => x.IdSchool == param.IdSchool)
            ...
            var sendSendGridEmailApi = await _sendGridEmailApi.SendSendGridEmail(new SendSendGridEmailRequest
            {
                ...
                RecepientConfiguration = new SendSendGridEmailRequest_RecepientConfiguration
                {
                    ...
                    BccList = approverDataEmail
                }
            });

            var result = new SendEmailCancelRequestToParentResult();
