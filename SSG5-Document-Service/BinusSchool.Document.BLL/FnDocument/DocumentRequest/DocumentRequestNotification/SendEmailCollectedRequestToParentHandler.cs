...
{
    public class SendEmailCollectedRequestToParentHandler : FunctionsHttpSingleHandler
    {
        // MsNotificationTemplate - DB SCHOOL
        private readonly string _notificationScenario = "DRE10";
        ...
        public SendEmailCollectedRequestToParentHandler(
        ...
        {
            var param = await Request.ValidateBody<SendEmailCollectedRequestToParentRequest, SendEmailCollectedRequestToParentValidator>();

            var result = await SendEmailCollectedRequestToParent(param);

            return Request.CreateApiResult2(result as object);
        }

        public async Task<SendEmailCollectedRequestToParentResult> SendEmailCollectedRequestToParent(SendEmailCollectedRequestToParentRequest param)
        {
            ...
            emailHtmlTemplate = emailHtmlTemplate
                                ...
                                .Replace("{{CollectedDate}}", documentRequestDetail.CollectionInfo == null ? "-" : documentRequestDetail.CollectionInfo.CollectedDate.Value.ToString("dd MMMM yyyy, HH:mm:ss"))
                                .Replace("{{CollectedBy}}", documentRequestDetail.CollectionInfo == null ? "-" : string.IsNullOrWhiteSpace(documentRequestDetail.CollectionInfo.CollectedBy) ? "-" : documentRequestDetail.CollectionInfo.CollectedBy)
            ...
            // approver
            if (approverDataList != null && approverDataList.Any())
                bccList.AddRange(approverDataList
            ...
            var result = new SendEmailCollectedRequestToParentResult();
