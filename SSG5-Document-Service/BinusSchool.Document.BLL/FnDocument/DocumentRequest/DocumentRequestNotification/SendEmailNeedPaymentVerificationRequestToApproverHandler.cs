...
{
    public class SendEmailNeedPaymentVerificationRequestToApproverHandler : FunctionsHttpSingleHandler
    {
        // MsNotificationTemplate - DB SCHOOL
        private readonly string _notificationScenario = "DRE03";
        ...
        public SendEmailNeedPaymentVerificationRequestToApproverHandler(
        ...
        {
            var param = await Request.ValidateBody<SendEmailNeedPaymentVerificationRequestToApproverRequest, SendEmailNeedPaymentVerificationRequestToApproverValidator>();

            var result = await SendEmailNeedPaymentVerificationRequestToApprover(param);

            return Request.CreateApiResult2(result as object);
        }

        public async Task<SendEmailNeedPaymentVerificationRequestToApproverResult> SendEmailNeedPaymentVerificationRequestToApprover(SendEmailNeedPaymentVerificationRequestToApproverRequest param)
        {
            ...
            // get data
            var documentRequestDetail = await _getDocumentRequestDetailHandler.GetDocumentRequestDetail(new GetDocumentRequestDetailRequest
            {
            ...
            emailHtmlTemplate = emailHtmlTemplate
                                ...
                                .Replace("{{PaymentDueDate}}", documentRequestDetail.Payment == null ? "-" : documentRequestDetail.Payment.EndDatePayment == null ? "-" : documentRequestDetail.Payment.EndDatePayment.Value.ToString("dd MMMM yyyy, HH:mm:ss"))
                                .Replace("{{InvoiceTotalAmount}}", documentRequestDetail.Payment == null ? "-" : string.Format("{0:N}", documentRequestDetail.Payment.TotalAmountInvoice))

                                .Replace("{{PaymentDate}}", documentRequestDetail.Payment == null ? "-" : documentRequestDetail.Payment.PaymentDate == null ? "-" : documentRequestDetail.Payment.PaymentDate.Value.ToString("dd MMMM yyyy, HH:mm:ss"))
                                .Replace("{{PaymentMethod}}", documentRequestDetail.Payment == null ? "-" : documentRequestDetail.Payment.DocumentReqPaymentMethod == null ? "-" : documentRequestDetail.Payment.DocumentReqPaymentMethod.Description)
                                .Replace("{{PaidAmount}}", documentRequestDetail.Payment == null ? "-" : documentRequestDetail.Payment.PaidAmount == null ? "-" : string.Format("{0:N}", documentRequestDetail.Payment.PaidAmount))
                                .Replace("{{SenderAccountName}}", documentRequestDetail.Payment == null ? "-" : string.IsNullOrEmpty(documentRequestDetail.Payment.SenderAccountName) ? "-" : documentRequestDetail.Payment.SenderAccountName)
                                .Replace("{{TransferEvidance}}", documentRequestDetail.Payment == null ? "-" : string.IsNullOrEmpty(documentRequestDetail.Payment.AttachmentImageUrl) ? "-" : string.Format("<a href='{0}'>{1}</a>", documentRequestDetail.Payment.AttachmentImageUrl, documentRequestDetail.Payment.AttachmentImageUrl))
                                ...
                                .Replace("{{RedirectLink}}", hostUrl.Trim() + "/DocumentRequest/PaymentVerification")
            ...
            var result = new SendEmailNeedPaymentVerificationRequestToApproverResult();
