...
{
    public class SendEmailNewRequestToParentHandler : FunctionsHttpSingleHandler
    {
        // MsNotificationTemplate - DB SCHOOL
        private readonly string _notificationScenario = "DRE01";
        ...
        public SendEmailNewRequestToParentHandler(
        ...
        {
            var param = await Request.ValidateBody<SendEmailNewRequestToParentRequest, SendEmailNewRequestToParentValidator>();

            var result = await SendEmailNewRequestToParent(param);

            return Request.CreateApiResult2(result as object);
        }

        public async Task<SendEmailNewRequestToParentResult> SendEmailNewRequestToParent(SendEmailNewRequestToParentRequest param)
        {
            ...
            if (documentRequestDetail.LatestDocumentReqStatusWorkflow.IdDocumentReqStatusWorkflow == DocumentRequestStatusWorkflow.WaitingForApproval)
            {
                additionalNotesHtml += "The status of your document request is <b>Waiting for Approval</b>, please wait for your request to be approved.";
            }
            else if(documentRequestDetail.LatestDocumentReqStatusWorkflow.IdDocumentReqStatusWorkflow == DocumentRequestStatusWorkflow.WaitingForPayment)
            {
    ...
    <tr>
        ...
        <td>{{PaymentInstruction}}</td>
            ...
            var result = new SendEmailNewRequestToParentResult();
