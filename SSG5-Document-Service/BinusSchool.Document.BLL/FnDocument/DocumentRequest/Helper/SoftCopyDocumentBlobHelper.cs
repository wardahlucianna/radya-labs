...
{
    public class SoftCopyDocumentBlobHelper
    {
        private readonly IConfiguration _configuration;
        private readonly IMachineDateTime _dateTime;

        private const string _containerNameInit = "document-request-softcopy";

        public SoftCopyDocumentBlobHelper(
        ...
        public string GetDocumentLink(string filename, string idStudent, int expiryHour = 1, string storedPolicyName = null)
        {
            if (string.IsNullOrEmpty(filename) || string.IsNullOrEmpty(idStudent))
                return null;

            string containerSasUri = GetContainerSasUri(idStudent, expiryHour, storedPolicyName);

            string url = containerSasUri.Replace("?", "/" + idStudent + "/" + filename + "?");
