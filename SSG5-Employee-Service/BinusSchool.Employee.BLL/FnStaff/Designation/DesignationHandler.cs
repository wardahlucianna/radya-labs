...
{
    public class DesignationHandler : FunctionsHttpSingleHandler
    {
        private readonly IEmployeeDbContext _dbContext;
        public DesignationHandler(IEmployeeDbContext employeeDbContext)
        {
        ...
        protected override async Task<ApiErrorResult<object>> Handler()
        {
            ...
            var predicate = PredicateBuilder.Create<LtDesignation>(x => true);
            if (!string.IsNullOrWhiteSpace(param.Search))
                predicate = predicate.And(x
                    => EF.Functions.Like(x.DesignationDescription, $"%{param.Search}%"));

            var query = _dbContext.Entity<LtDesignation>()
                .Where(predicate)
            ...
            items = await query
                .Select(x => new ItemValueVm
                {
                    ...
                    Description = x.DesignationDescription
                })
