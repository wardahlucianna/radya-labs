...
{
    public class UpdateStaffCertificationHandler : FunctionsHttpSingleHandler
    {
        private readonly IEmployeeDbContext _dbContext;
        private IDbContextTransaction _transaction;
        private readonly IMachineDateTime _dateTime;
        public UpdateStaffCertificationHandler(
        ...
        protected override async Task<ApiErrorResult<object>> Handler()
        {
            try
            {
                ...
                {
                    var getdata = await _dbContext.Entity<TrStaffCertificationInformation>().Where(p => p.IdCertificationStaff == body.Constraint3Value && p.IdBinusian == body.IdBinusian).FirstOrDefaultAsync();
                    if (getdata is null)
                        throw new BadRequestException(string.Format(Localizer["ExNotExist"], Localizer["Id Certification Staff"], "Id", body.Constraint3Value));
