...
{
    public class UpdateStaffHandler : FunctionsHttpSingleHandler
    {
        private readonly IEmployeeDbContext _dbContext;
        private IDbContextTransaction _transaction;
        private readonly IMachineDateTime _dateTime;

        public UpdateStaffHandler(IEmployeeDbContext dbContext, IMachineDateTime dateTime)
        {
        ...
        protected override async Task<ApiErrorResult<object>> Handler()
        {
            try
            {
                ...
                {
                    var getdata = await _dbContext.Entity<MsStaff>().Where(p => p.IdBinusian == body.IdBinusian).FirstOrDefaultAsync();
                    if (getdata is null)
                        throw new BadRequestException(string.Format(Localizer["ExNotExist"], Localizer["Id Binusian"], "Id", body.Constraint3Value));
