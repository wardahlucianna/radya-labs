...
{
    public class GetMasterExtracurricularHandler : FunctionsHttpSingleHandler
    {
        ...
        public GetMasterExtracurricularHandler(ISchedulingDbContext dbContext, IMachineDateTime dateTime, GetPrivilegeUserElectiveHandler getPrivilegeUserElectiveHandler)
        {
        ...
        {
            var param = Request.ValidateParams<GetMasterExtracurricularRequest>(_requiredParams);
            ...
            if (param.IdGrade != null)
                predicate = predicate.And(x => x.ExtracurricularGradeMappings.Any(y => y.Grade.Id == param.IdGrade));
            ...
            //    predicate = predicate.And(x => x.ExtracurricularSessionMappings.Select(x => x.ExtracurricularSession.IdDay).ToList().Any(y => y == param.ScheduleDay));
            if (!string.IsNullOrWhiteSpace(param.IdUser) && getUser != null)
            ...
            { 
                ...
                ElectivesGrade = x.ExtracurricularGradeMappings
                    ...
                    { Id =  x.IdGrade, Description = x.Grade.Description, OrderNumber = x.Grade.OrderNumber
                    })
            ...
            if (param.Return == CollectionType.Lov)
            {
                var result = await query
                    //.Where(x => dataElectivesSearch.Any(y => y == x.Id))
                    .Select(x => new ItemValueVm { 
                        Id = x.Id,
                        Description = x.Name + "-" + string.Join("; ", x.ExtracurricularGradeMappings.OrderBy(a => a.Grade.OrderNumber).Select(a => a.Grade.Description))
                    })
