...
{
    public class GetTeacherVenueMappingHandler : FunctionsHttpSingleHandler
    {

        private readonly ISchedulingDbContext _dbContext;

        public GetTeacherVenueMappingHandler(ISchedulingDbContext schoolDbContext)
        {
        ...
        protected override async Task<ApiErrorResult<object>> Handler()
        {
            ...
            var HomeroomTeacher = await _dbContext.Entity<MsHomeroomTeacher>()
                            ...
                            && (e.Homeroom.Grade.Level.AcademicYear.IdSchool != "3"))
            ...
            //Subject Teacher
            var GetIdTeacherPosition = InvitationBookingSettingVenue.Where(e=>e.TeacherPosition.Position.Code=="ST").Select(e=>e.IdTeacherPosition).ToList();
            var GetTeacherPositionBySubject = await _dbContext.Entity<MsTeacherPosition>()
                                                .Include(e => e.Position)
                                               .Where(e => e.Position.Code == "ST" && e.IdSchool== IdSchool && GetIdTeacherPosition.Contains(e.Id))
            ...
            if (GetTeacherPositionBySubject != null)
            {
            ...
            if (IdSchool == "3")
            {
                getPositionByUser = getPositionByUser.Where(e => e.MsNonTeachingLoad.TeacherPosition.Code == PositionConstant.GuidanceCounselling).ToList();
            }
            ...
            foreach (var itemHomeroomStudent in GetHomeroomStudents.Select(e => new { e.IdGrade, e.IdLevel }).Distinct().ToList())
            {
                ...
                if (getPositionByUser.Where(y => y.MsNonTeachingLoad.TeacherPosition.Position.Code == PositionConstant.SubjectHead).ToList() != null)
                {
                    ...
                    foreach (var item in SubjectHead)
                    {
                        ...
                        if (_GradeSH.Id == itemHomeroomStudent.IdGrade && _DepartementSH != null)
                        {
                            idGrades.AddRange(await _dbContext.Entity<MsGrade>().Include(e => e.Level).Where(x => x.IdLevel == itemHomeroomStudent.IdLevel)
                            .Select(x => new GetUserGrade
                            {
            ...
            if (IdSchool == "2")
            {
                ...
                NewUserTeacher = NewUserTeacher.Where(e => listIdTeacherPosition.Contains(e.IdTeacherPosition) && GetUserRole.Contains(e.IdUser)).ToList();
            }

            var items = GetTeacher(NewUserTeacher);
            //var items = NewUserTeacher.Where(e => InvitationBookingSettingVenue.Contains(e.IdUser)).Distinct().ToList();
