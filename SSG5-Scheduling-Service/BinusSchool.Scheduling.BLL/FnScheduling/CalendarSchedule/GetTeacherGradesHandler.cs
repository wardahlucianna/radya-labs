...
//             var gradeIds = await _dbContext.Entity<TrGeneratedScheduleLesson>()
//                                            .Include(x => x.GeneratedScheduleStudent).ThenInclude(x => x.GeneratedScheduleGrade)
//                                            .Where(x => x.IdUser == param.IdUser)
//                                            .Select(x => x.GeneratedScheduleStudent.GeneratedScheduleGrade.IdGrade)
//                                            .Distinct()
...
//             else if (!string.IsNullOrWhiteSpace(param.IdAcadYear))
