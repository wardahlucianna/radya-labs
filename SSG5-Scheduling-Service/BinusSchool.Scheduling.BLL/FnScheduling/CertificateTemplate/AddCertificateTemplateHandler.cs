...
{
    public class AddCertificateTemplateHandler : FunctionsHttpSingleHandler
    {
        private readonly ISchedulingDbContext _dbContext;

        public AddCertificateTemplateHandler(ISchedulingDbContext schedulingDbContext)
        {
        ...
        {
            var body = await Request.ValidateBody<AddCertificateTempRequest, AddCertificateTemplateValidator>();
            ...
            var isNameExist = await _dbContext.Entity<MsCertificateTemplate>()
                .Where(x => x.IdAcademicYear  == body.IdAcademicYear && x.Name.ToLower() == body.Name.ToLower())
                .FirstOrDefaultAsync(CancellationToken);
            if (isNameExist != null)
            ...
            var certificateTemplate = new MsCertificateTemplate
            {
                Id = Guid.NewGuid().ToString(),
                IdAcademicYear  = body.IdAcademicYear,
                Name = body.Name,
                IsUseBinusLogo = body.IsUseBinusLogo,
                Title = body.Title,
                SubTitle = body.SubTitle,
                Description = body.Description,
                Background = body.Background,
                Signature1 = body.Signature1,
                Signature1As = body.Signature1As,
                Signature2 = body.Signature2 != null ? body.Signature2 : null,
                Signature2As = body.Signature2 != null ? body.Signature2As : null,
                ApprovalStatus = "On Review"
            };

            _dbContext.Entity<MsCertificateTemplate>().Add(certificateTemplate);
            ...
            if (dataPrincipal == null)
                throw new NotFoundException("User position Principal/Vice Principal in this school not found");
            ...
            var dataStaff = _dbContext.Entity<MsUser>()
                .FirstOrDefault(x => x.Id == "BN123502013");

            await _dbContext.SaveChangesAsync(CancellationToken);

            await SendEmailAndNotif(certificateTemplate, dataPrincipal.DisplayName, dataStaff.DisplayName);
        ...
        public async Task SendEmailAndNotif(MsCertificateTemplate msCertificateTemplate, string NameUserApproval, string NameStaff)
        {
            if (!string.IsNullOrEmpty(NameUserApproval))
            {
                ...
                {
                    var message = JsonConvert.SerializeObject(new NotificationQueue(AuthInfo.IdCurrentTenant, "EM6")
                    {
