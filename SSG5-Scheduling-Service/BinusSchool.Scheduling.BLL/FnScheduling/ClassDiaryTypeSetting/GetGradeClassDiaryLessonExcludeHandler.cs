...
{
    public class GetGradeClassDiaryLessonExcludeHandler : FunctionsHttpSingleHandler
    {

        private readonly ISchedulingDbContext _dbContext;

        public GetGradeClassDiaryLessonExcludeHandler(ISchedulingDbContext dbContext)
        {
        ...
        {
            var param = Request.ValidateParams<GetGradeClassDiaryTypeSettingRequest>();

            var query = await _dbContext.Entity<MsClassDiaryLessonExclude>()
                .Include(p => p.Lesson).ThenInclude(p => p.Grade)
                .Where(x => x.IdClassDiaryTypeSetting == param.IdClassDiaryTypeSetting)
                .ToListAsync(CancellationToken);

            var Grades = query
                .GroupBy(x => x.Lesson.Grade)
