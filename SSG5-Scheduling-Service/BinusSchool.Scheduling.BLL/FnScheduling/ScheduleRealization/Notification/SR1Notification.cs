...
{
    public class SR1Notification : FunctionsNotificationHandler
    {
        private IDictionary<string, object> _notificationData;
        private readonly ILogger<SR1Notification> _logger;
        private readonly ISchedulingDbContext _dbContext;
        private readonly IConfiguration _configuration;

        private string _schoolName;

        public SR1Notification(INotificationManager notificationManager, IConfiguration configuration, ILogger<SR1Notification> logger, ISchedulingDbContext dbContext, IDictionary<string, object> notificationData) :
           base("SR1", notificationManager, configuration, logger)
        {
            _dbContext = dbContext;
            _configuration = configuration;
            _logger = logger;

            PushNotificationData["action"] = "SR_SUBSTITUTE_TEACHER";
        }
        ...
        protected override async Task SendEmailNotification()
        {
            ...
            var GetUserTeacher = await _dbContext.Entity<MsUser>()
                .Where(x => EmailScheduleRealization.IdUserTeacher.Contains(x.Id) && x.Status)
                .Select(x => new
                {
                    x.Id,
                    EmailAddress = new EmailAddress(x.Email, x.DisplayName),
            ...
            var listTeacherOld = GetUserTeacher.Select(x => x.Id).Distinct().ToList();
            ...
            var emailUserCC = await _dbContext.Entity<MsUser>()
                .Where(x => IdUserSpesific.Distinct().Contains(x.Id) && !GetUser.Select(x => x.Id).ToList().Contains(x.Id) && x.Status)
                .Select(x => new
                {
                    x.Id,
                    EmailAddress = new EmailAddress(x.Email, x.DisplayName),
