...
{
    public class SR8Notification : FunctionsNotificationHandler
    {
        private IDictionary<string, object> _notificationData;
        private readonly ILogger<SR8Notification> _logger;
        ...
        public SR8Notification(INotificationManager notificationManager, IRolePosition rolePosition, IConfiguration configuration, ILogger<SR8Notification> logger, ISchedulingDbContext dbContext, IDictionary<string, object> notificationData) :
           base("SR8", notificationManager, configuration, logger)
        {
        ...
        protected override async Task SendEmailNotification()
        {
            try
            {
                ...
                foreach (var idUser in IdUserRecipients)
                {
                    ...
                    {
                        { "SubstituteTeacherName",userEmail.DisplayName },
                        { "DateRange",_EmailScheduleRealization.Date?.ToString("dd MMMM yyyy") },
                        ...
                        { "SubstituteTeacher", string.Join(",", GetSubtituteUserTeacher.Select(x => x.Name).ToList())},
                        { "ChangeVenue", _EmailScheduleRealization.IdChangeVenue == null ? dataVenue.Where(x => x.Id == _EmailScheduleRealization.IdRegularVenue).First().Description : dataVenue.Where(x => x.Id == _EmailScheduleRealization.IdChangeVenue).First().Description},
                        { "NotesforSubstituteTeacher", _EmailScheduleRealization.NotesForSubtitutions },
        ...
        protected override async Task SaveNotification(IEnumerable<string> idUserRecipients, bool isBlast)
        {
            try
            {
                ...
                foreach (var idUser in IdUserRecipients)
                {
                    ...
                    {
                        GeneratedContent = $"There is an undo schedule realization. Please check your email for the details.";
                    }
                    else
                    {
