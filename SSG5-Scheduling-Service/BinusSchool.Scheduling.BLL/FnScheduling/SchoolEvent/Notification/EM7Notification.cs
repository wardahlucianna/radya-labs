...
{
    public class EM7Notification : FunctionsNotificationHandler
    {
        ...
        public EM7Notification(string idScenario, INotificationManager notificationManager, IConfiguration configuration, 
            ILogger<FunctionsNotificationHandler> logger, ISchedulingDbContext dbContext, IDictionary<string, object> notificationData) : base("EM7", notificationManager, configuration, logger)
        {
        ...
        {
            var IdCertificateTemplate = KeyValues.Where(e => e.Key == "IdCertificateTemplate").Select(e => e.Value).SingleOrDefault().ToString();

            var url = $"{_configuration.GetSection("ClientApp:Web:Host").Get<string>()}/certificatesetting/certificateapproval";
            var urlApprove = $"{_configuration.GetSection("ClientApp:Web:Host").Get<string>()}/certificatesetting/certificateapprovaldetail?id={IdCertificateTemplate}";
            var urlDecline = $"{_configuration.GetSection("ClientApp:Web:Host").Get<string>()}/certificatesetting/certificateapprovaldetail?id={IdCertificateTemplate}";
            ...
            {
                { "Link", url },
                { "LinkApprove", urlApprove },
                { "LinkDecline", urlDecline },
