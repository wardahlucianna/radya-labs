...
namespace BinusSchool.Scheduling.FnLongRun.BlobTrigger.AscTimetable
{
    public class AddAscTimetableHandler
    {
        ...
        private readonly ITeacherPosition _teacherPosition;
        private readonly IMessage _messageService;
        private readonly IAscTimetable _ascService;
        ...
        public AddAscTimetableHandler(ISchedulingDbContext dbContext,
            ...
            ITeacherPosition teacherPosition,
            IAscTimetable ascService,
        ...
        public async Task AddAscTimetable([BlobTrigger(_blobPath)] Stream blobStream,
            ...
            string name,
            [Queue("notification-asc-longrun")] ICollector<string> collector,
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                // set job started
                var startProcess = await _ascService.StartProcess(new StartAscTimetableProcessRequest
                {
                    IdSchool = _idSchool,
                ...
                var getposition = await _teacherPosition.GetPositionCAforAsc(new Data.Model.Teaching.FnAssignment.TeacherPosition.GetCAForAscTimetableRequest
                {
                ...
                if (body.IsCreateSessionSetFromXml)
                {
                    ...
                    ascTimetableId = await AddAscTimeTable(new TrAscTimetable()
                    {
                        ...
                        IdGradePathwayForParticipant = JsonConvert.SerializeObject(body.IdGradepathwayforCreateSession),
                ...
                await SaveDatas(body, ascTimetableId, semseter, getposition.Payload, idSessionSet);
                ...
                process.FinishedAt = _datetime.ServerTime;
                ...
                // send notification
                var queueMessage = JsonConvert.SerializeObject(new NotificationQueue(_idSchool, "ASC1")
                {
            ...
            catch (Exception ex)
            {
                ...
                if (!string.IsNullOrEmpty(idProcess))
                    _ = await _ascService.FinishProcess(new FinishAscTimetableProcessRequest
                    {
                        IdProcess = idProcess
                    });
                ...
                // send notification
                var queueMessage = JsonConvert.SerializeObject(new NotificationQueue(_idSchool, "ASC2")
                {
                    IdRecipients = new[] { _idUser },
                    KeyValues = new Dictionary<string, object> { { "date", DateTimeUtil.ServerTime } }
                });
                ...
                var queueErrorMessage = JsonConvert.SerializeObject(new NotificationQueue(_idSchool, "ASC0")
                {
        ...
        /// <returns></returns>
        private async Task SaveDatas(AddDataAscTimeTableAfterUploadRequest model, string ascTimetableId, Dictionary<string, List<int>> Semesters, string CaPosition, string idSessionSet)
        {
            ...
            var gradeIds = model.Lesson.Select(p => p.Grade).GroupBy(p => p.IdFromMasterData).Select(p => p.Key).ToList();
            ...
            var getHomeroomPathwaysFromDB = await _dbContext.Entity<MsHomeroomPathway>()
                                                           ...
                                                           .Where(p => getdataPathwaysFromXml.Any(x => x == p.IdGradePathwayDetail) &&
                                                                       ...
                                                                       gradeIds.Contains(p.Homeroom.IdGrade))
                                                           .ToListAsync();
            ...
            var getLessonFromDB = await _dbContext.Entity<MsLesson>()
                                                  ...
                                                  .Where(p => classIdFromXml.Any(x => x == p.ClassIdGenerated) &&
                                                              ...
                                                              gradeIds.Contains(p.IdGrade) &&
                                                              p.IdAcademicYear == model.IdSchoolAcademicyears)
                                                  .ToListAsync();

            var sessionIds = await _dbContext.Entity<MsSession>()
                                             .Where(x => x.IdSessionSet == idSessionSet)
                                             .Select(x => x.Id)
                                             .ToListAsync();

            var homeroomStudent = getHomeroomPathwaysFromDB.SelectMany(p => p.Homeroom.HomeroomStudents).ToList();

            var idHomeroomStudentList = homeroomStudent.Select(x => x.Id).Distinct().ToList();

            var getLessonId = getLessonFromDB.Select(x => x.Id).ToList();

            var getHomeroomStudentEnrollmentFromDB = await _dbContext.Entity<MsHomeroomStudentEnrollment>()
                                 .Where(p => idHomeroomStudentList.Contains(p.IdHomeroomStudent) &&
                                                getLessonId.Any(x => x == p.IdLesson) &&
                                                subjectFromXml.Any(x => x == p.IdSubject))
                                 .ToListAsync();
            ...
            {
                var datasemester = new List<int>();
                if (Semesters.TryGetValue(grade, out datasemester))
                {
                    foreach (var itemSemester in datasemester)
                    {
                        SaveLessonNew(model, ascTimetableId, itemSemester, grade, getLessonFromDB, getHomeroomPathwaysFromDB, weekVarian, sessionIds, getHomeroomStudentEnrollmentFromDB);
                    }
                }
            }
            await _dbContext.SaveChangesAsync(_ignoredTables.Value, _cancellationToken);
        ...
        /// <returns></returns>
        public void SaveLessonNew(AddDataAscTimeTableAfterUploadRequest model,
                                  ...
                                  List<MsHomeroomStudentEnrollment> homeroomStudentEnrollmentFromDB)
        {
            ...
            foreach (var itemLesson in model.Lesson.Where(p => p.Grade.IdFromMasterData == idGrade))
            {
                ...
                if (model.SaveLesson)
                {
                    ...
                    if (getData != null)
                    {
                        ...
                        SaveSchedule(model, ascTimetableId, getData.ClassIdGenerated, Semester, getData.Id, getData.IdSubject, getData.IdWeekVariant, new List<string>(), false, weekvarianDetail, sessionIds);
                        SaveEnrollment(model, ascTimetableId, getData.ClassIdGenerated, Semester, getData.Id, itemLesson.IdForSave, homeroomStudent, homeroomStudentEnrollmentFromDB);
                    ...
                    else
                    {
                        ...
                        SaveSchedule(model, ascTimetableId, itemLesson.ClassId, Semester, dataLesson.Id, dataLesson.IdSubject, dataLesson.IdWeekVariant, new List<string>(), false, weekvarianDetail, sessionIds);
                        SaveEnrollment(model, ascTimetableId, itemLesson.ClassId, Semester, dataLesson.Id, itemLesson.IdForSave, homeroomStudent, homeroomStudentEnrollmentFromDB);
                ...
                else
                {
                    ...
                    if (getData != null)
                    {
                        ...
                        SaveSchedule(model, ascTimetableId, itemLesson.ClassId, Semester, getData.Id, getData.IdSubject, getData.IdWeekVariant, teacherLesson, true, weekvarianDetail, sessionIds);
                        SaveEnrollment(model, ascTimetableId, itemLesson.ClassId, Semester, getData.Id, itemLesson.IdForSave, homeroomStudent, homeroomStudentEnrollmentFromDB);
                    }
        ...
        public void SaveSchedule(AddDataAscTimeTableAfterUploadRequest model,
                                               string ascTimetableId,
                                               string classId,
                                               int Semester,
                                               string lessonId,
                                               ...
                                               List<string> sessionIds)
        {
            if (model.SaveSchedule)
            {
                ...
                foreach (var item in getData)
                {
                    ...
                    {
                        if (sessionIds.Contains(item.data.IdDataFromMaster))
                        {
        ...
        public string SaveEnrollment(AddDataAscTimeTableAfterUploadRequest model, string ascTimetableId, string classId, int Semester, string lessonId, string lessonIdFromXMl, List<MsHomeroomStudent> student, List<MsHomeroomStudentEnrollment> homeroomstudentEnrollment)
        {
                ...
                foreach (var itemEnroll in model.Enrollment.EnrollmentData.Where(p => p.EnrollmentStudent.Any(x => x.ClassId == classId && x.LessonId == lessonIdFromXMl)))
                {
                    ...
                    foreach (var item in itemEnroll.EnrollmentStudent.Where(p => p.ClassId == classId && p.LessonId == lessonIdFromXMl))
                    {
                        if (item.IsDataReadyFromMaster)
                        {
                            ...
                            {
                                var idHomeroomStudentEnrollment = homeroomstudentEnrollment.Where(x => x.IdLesson == lessonId && x.IdSubject == item.SubjectID
                                   && x.IdHomeroomStudent == getStudentHomeroom.Id).Select(x => x.Id).FirstOrDefault();

                                if (idHomeroomStudentEnrollment == null)
                                {
                                ...
                                {
                                    SaveEnrolmentAsc(ascTimetableId, idHomeroomStudentEnrollment);
                                }
                            }
                        }
                    }
