...
namespace BinusSchool.Scheduling.FnLongRun.BlobTrigger.Scheduling
{
    ...
    {
#if DEBUG
        private const string _blobPath = "generate-schedule-debug/add1/{name}.json";
#else
        private const string _blobPath = "generate-schedule/add1/{name}.json";
#endif
                ...
                if (body.Type == "grade")
                {
                    ...
                    foreach (var grade in body.Grades)
                    {
                        ...
                        foreach (var studentEnrollment in students)
                        {
                            ...
                            .Select(x => x.ClassIdGenerated)
                            .ToList();
                            var classId = lessons.GroupBy(x => x.ClassId).Select(x => x.Key).ToList();
                            var finalClassId = classId.Intersect(enrollmentStudent).ToList();
                            studentEnrollment.Lessons = lessons.Where(x => finalClassId.Any(y => y == x.ClassId)).ToList();
                    ...
                    foreach (var grade in body.Grades)
                    {
                        var _grade = await _dbContext.Entity<MsGrade>()
                        ...
                        foreach (var student in grade.Students)
                        {
                            ...
                            var generatedScheduleStudent = await _dbContext.Entity<TrGeneratedScheduleStudent>().Where(x => x.IdStudent == student.StudentId &&
                            x.IdGenerateScheduleGrade == trGeneratedScheduleGrade.Id).FirstOrDefaultAsync();
                            ...
                            foreach (var lesson in student.Lessons)
                            {
                                ...
                                var enrollmentData = homeroomByStudentAndLessons.Where(x => x.IdGrade == trGeneratedScheduleGrade.IdGrade
                                                                                            && x.IdStudent == student.StudentId
                                                                                            && x.ClassIdGenerated == lesson.ClassId
                                                                                            && x.Semester == semesterByDate)
                                                                                .FirstOrDefault();
                                if (enrollmentData != null)
                                {
                                    _dbContext.Entity<TrGeneratedScheduleLesson>().Add(new TrGeneratedScheduleLesson
                                    {
                                        Id = Guid.NewGuid().ToString(),
                                        IdGeneratedScheduleStudent = generatedScheduleStudent.Id,
                                        IdWeek = lesson.WeekId,
                                        IdUser = lesson.TeacherId,
                                        TeacherName = lesson.TeacherName,
                                        IdVenue = lesson.VenueId,
                                        VenueName = lesson.VenueName,
                                        ClassID = lesson.ClassId,
                                        IdLesson = enrollmentData.IdLesson,
                                        IsGenerated = true,
                                        ScheduleDate = lesson.ScheduleDate,
                                        StartPeriod = lesson.StartPeriode,
                                        EndPeriod = lesson.EndPeriode,
                                        ...
                                        IdHomeroom = enrollmentData.IdHomeroom,
                                        HomeroomName = enrollmentData.HomeroomName,
                                        IdBinusianOld = lesson.TeacherId,
                                        TeacherNameOld = lesson.TeacherName,
                                        IdVenueOld = lesson.VenueId,
                                        VenueNameOld = lesson.VenueName,
                                        IsCancelScheduleRealization = false,
                                        IsSetScheduleRealization = false,
                                        IsChangeEvent = false,
                                        IdAcademicYear = _grade.Level.IdAcademicYear,
                                        IdGrade = _grade.Id,
