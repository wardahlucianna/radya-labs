...
{
    public class EM1Notification : FunctionsNotificationHandler
    {
        ...
        public EM1Notification(string idScenario, INotificationManager notificationManager, IConfiguration configuration, 
            ILogger<FunctionsNotificationHandler> logger, ISchedulingDbContext dbContext, IDictionary<string, object> notificationData) : base("EM1", notificationManager, configuration, logger)
        {             
        ...
        {
            var IdEvent = KeyValues.Where(e => e.Key == "IdEvent").Select(e => e.Value).SingleOrDefault().ToString();

            var url = $"{_configuration.GetSection("ClientApp:Web:Host").Get<string>()}/schoolevent/list";
            var urlDetail = $"{_configuration.GetSection("ClientApp:Web:Host").Get<string>()}/schoolEvent/detail?id={IdEvent}";
