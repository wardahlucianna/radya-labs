...
using BinusSchool.School.FnSchool.Building.Validator;
using Microsoft.EntityFrameworkCore;

namespace BinusSchool.School.FnSchool.Building
{
    public class BuildingHandler : FunctionsHttpCrudHandler
    {
        private readonly ISchoolDbContext _dbContext;
        public BuildingHandler(ISchoolDbContext schoolDbContext)
        {
        ...
        protected override async Task<ApiErrorResult<object>> DeleteHandler(IEnumerable<string> ids)
        {
            ...
            var datas = await _dbContext.Entity<MsBuilding>()
                .Include(x => x.Venues)
            ...
            foreach (var data in datas)
            {
                ...
                {
                    data.IsActive = false;
                    _dbContext.Entity<MsBuilding>().Update(data);
                }
            }
        ...
        {
            var data = await _dbContext.Entity<MsBuilding>()
                .Include(x => x.School)
            ...
            if (data != null)
            {
                var usedVenue = _dbContext.Entity<MsVenue>().Any(x=> x.IdBuilding == data.Id);

                data.CanEditName = !usedVenue;
            }
        ...
        protected override async Task<ApiErrorResult<IReadOnlyList<IItemValueVm>>> GetHandler()
        {
            ...
            var predicate = PredicateBuilder.Create<MsBuilding>(x => param.IdSchool.Any(y => y == x.IdSchool));
            if (!string.IsNullOrWhiteSpace(param.Search))
                predicate = predicate.And(x
                    => EF.Functions.Like(x.Description, param.SearchPattern())
                    || EF.Functions.Like(x.Code, param.SearchPattern()));

            var query = _dbContext.Entity<MsBuilding>()
                .Include(x => x.Venues)
                .Where(predicate)
            ...
            if (param.Return == CollectionType.Lov)
                items = await query
                    .Select(x => new ItemValueVm(x.Id, x.Code))
                    .ToListAsync(CancellationToken);
            else
                items = await query
                    .SetPagination(param)
                    .Select(x => new ListBuildingResponse
                    {
                        Id = x.Id,
                        Code = x.Code,
                        Description = x.Description,
                        CanModified = x.Venues.Any() ? false : true
                    })
                    .ToListAsync(CancellationToken);
        ...
        {
            var body = await Request.ValidateBody<AddBuildingRequest, AddBuildingValidator>();
            Transaction = await _dbContext.BeginTransactionAsync(CancellationToken);
            ...
            var isExist = await _dbContext.Entity<MsBuilding>()
                .Where(x => x.IdSchool == body.IdSchool && x.Code.ToLower() == body.Code.ToLower())
                .FirstOrDefaultAsync();
            if (isExist != null)
                throw new BadRequestException($"{body.Code} already exists");

            var param = new MsBuilding
            {
            ...
            _dbContext.Entity<MsBuilding>().Add(param);
        ...
        {
            var body = await Request.ValidateBody<UpdateBuildingRequest, UpdateBuildingValidator>();
            Transaction = await _dbContext.BeginTransactionAsync(CancellationToken);

            var data = await _dbContext.Entity<MsBuilding>().FindAsync(body.Id);
            if (data is null)
            ...
            var isExist = await _dbContext.Entity<MsBuilding>()
                .Where(x => x.Id != body.Id && x.IdSchool == body.IdSchool && x.Code.ToLower() == body.Code.ToLower())
                .FirstOrDefaultAsync();
            if (isExist != null)
                throw new BadRequestException($"{body.Code} already exists");
