...
using BinusSchool.School.FnSchool.ClassRoom.Validator;
using Microsoft.EntityFrameworkCore;

namespace BinusSchool.School.FnSchool.ClassRoom
{
    public class ClassRoomsHandler : FunctionsHttpCrudHandler
    {
        private readonly ISchoolDbContext _dbContext;
        public ClassRoomsHandler(ISchoolDbContext schoolDbContext)
        {
        ...
        protected override async Task<ApiErrorResult<object>> DeleteHandler(IEnumerable<string> ids)
        {
            ...
            var datas = await _dbContext.Entity<MsClassroom>()
                .Include(x => x.GradePathwayClassrooms)
            ...
            foreach (var data in datas)
            {
                ...
                if (data.GradePathwayClassrooms.Count != 0)
                {
                ...
                {
                    data.IsActive = false;
                    _dbContext.Entity<MsClassroom>().Update(data);
                }
            }
        ...
        {
            var data = await _dbContext.Entity<MsClassroom>()
                .Include(x => x.School)
        ...
        protected override async Task<ApiErrorResult<IReadOnlyList<IItemValueVm>>> GetHandler()
        {
            ...
            var predicate = PredicateBuilder.Create<MsClassroom>(x => param.IdSchool.Any(y => y == x.IdSchool));
            if (!string.IsNullOrWhiteSpace(param.Search))
                predicate = predicate.And(x
                    => EF.Functions.Like(x.Code, $"%{param.Search}%")
                    || EF.Functions.Like(x.Description, $"%{param.Search}%"));

            var query = _dbContext.Entity<MsClassroom>()
                .Where(predicate)
        ...
        {
            var body = await Request.ValidateBody<AddClassRoomRequest, AddClassRoomValidator>();
            Transaction = await _dbContext.BeginTransactionAsync(CancellationToken);
            ...
            var isExist = await _dbContext.Entity<MsClassroom>()
            ...
            _dbContext.Entity<MsClassroom>().Add(param);
        ...
        {
            var body = await Request.ValidateBody<UpdateClassRoomRequest, UpdateClassRoomValidator>();
            Transaction = await _dbContext.BeginTransactionAsync(CancellationToken);

            var data = await _dbContext.Entity<MsClassroom>().FindAsync(body.Id);
            if (data is null)
