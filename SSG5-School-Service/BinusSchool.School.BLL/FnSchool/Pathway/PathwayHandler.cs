...
{
    public class PathwayHandler : FunctionsHttpCrudHandler
    {
        private readonly ISchoolDbContext _dbContext;
        public PathwayHandler(ISchoolDbContext schoolDbContext)
        {
        ...
        protected override async Task<ApiErrorResult<object>> DeleteHandler(IEnumerable<string> ids)
        {
            ...
            var datas = await _dbContext.Entity<MsPathway>()
                .Include(x => x.GradePathwayDetails)
            ...
            foreach (var data in datas)
            {
                ...
                if (data.GradePathwayDetails.Count != 0)
                {
                ...
                {
                    data.IsActive = false;
                    _dbContext.Entity<MsPathway>().Update(data);
                }
            }
        ...
        {
            var data = await _dbContext.Entity<MsPathway>()
                .Include(x => x.AcademicYear)
                .ThenInclude(x => x.School)
                .Select(x => new GetPathwayDetailResult
                {
                    Id = x.Id,
                    Code = x.Code,
        ...
        {
            var param = Request.ValidateParams<GetPathwayRequest>(nameof(GetPathwayRequest.IdSchool));
            ...
            var predicate = PredicateBuilder.Create<MsPathway>(x => param.IdSchool.Any(y => y == x.AcademicYear.IdSchool));
            ...
            if (!string.IsNullOrEmpty(param.IdPathway))
                predicate = predicate.And(x => x.Id == param.IdPathway);

            var query = _dbContext.Entity<MsPathway>()
                .SearchByIds(param)
                .Where(predicate)
        ...
        {
            var body = await Request.ValidateBody<AddPathwayRequest, AddPathwayValidator>();
            Transaction = await _dbContext.BeginTransactionAsync(CancellationToken);
            ...
            var isExist = await _dbContext.Entity<MsPathway>()
                .Where(x => x.IdAcademicYear == body.IdAcadyear && x.Code.ToLower() == body.Code.ToLower())
                .FirstOrDefaultAsync();
            if (isExist != null)
                throw new BadRequestException($"{body.Code} already exists in this academic year");

            var param = new MsPathway
            {
            ...
            _dbContext.Entity<MsPathway>().Add(param);
        ...
        {
            var body = await Request.ValidateBody<UpdatePathwayRequest, UpdatePathwayValidator>();
            Transaction = await _dbContext.BeginTransactionAsync(CancellationToken);

            var data = await _dbContext.Entity<MsPathway>().FindAsync(body.Id);
            if (data is null)
            ...
            var isExist = await _dbContext.Entity<MsPathway>()
                .Where(x => x.Id != body.Id && x.IdAcademicYear == body.IdAcadyear && x.Code.ToLower() == body.Code.ToLower())
                .FirstOrDefaultAsync();
            if (isExist != null)
                throw new BadRequestException($"{body.Code} already exists in this academic year");

            data.Code = body.Code;
            data.Description = body.Code;
