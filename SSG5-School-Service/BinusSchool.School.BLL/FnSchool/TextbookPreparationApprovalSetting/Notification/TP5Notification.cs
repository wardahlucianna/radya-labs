...
namespace BinusSchool.School.FnSchool.TextbookPreparationApprovalSetting.Notification
{
    public class TP5Notification : FunctionsNotificationHandler
    {
        private IDictionary<string, object> _notificationData;
        private readonly ILogger<TP5Notification> _logger;
        private readonly ISchoolDbContext _dbContext;
        private readonly IConfiguration _configuration;

        public TP5Notification(INotificationManager notificationManager, IConfiguration configuration, ILogger<TP5Notification> logger, ISchoolDbContext dbContext, IDictionary<string, object> notificationData) :
           base("TP5", notificationManager, configuration, logger)
        {
            _dbContext = dbContext;
            _configuration = configuration;
            _logger = logger;

            PushNotificationData["action"] = "TP_APPROVER";
        }
        ...
        protected override Task Prepare()
        {
            try
            {
                var UrlBase = $"{_configuration.GetSection("ClientApp:Web:Host").Get<string>()}/textbooksetting/approvalsetting";
        ...
        protected override async Task SendPushNotification()
        {
            ...
            _notificationData["Administrator"] = EmailApprovalSetting.NamaAdministrator;
        ...
        protected override async Task SendEmailNotification()
        {
            ...
            foreach (var IdUser in EmailApprovalSetting.IdUser)
            {
                ...
                _notificationData["Staff"] = User.EmailAddress.Name;
                _notificationData["Link"] = $"{_notificationData["UrlBase"]}";
