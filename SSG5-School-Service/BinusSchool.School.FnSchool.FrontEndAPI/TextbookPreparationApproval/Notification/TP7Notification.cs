...
{
    public class TP7Notification : FunctionsNotificationHandler
    {
        private IDictionary<string, object> _notificationData;
        private readonly ILogger<TP7Notification> _logger;
        private readonly ISchoolDbContext _dbContext;
        private readonly IConfiguration _configuration;

        public TP7Notification(INotificationManager notificationManager, IConfiguration configuration, ILogger<TP7Notification> logger, ISchoolDbContext dbContext, IDictionary<string, object> notificationData) :
           base("TP7", notificationManager, configuration, logger)
        {
            _dbContext = dbContext;
            _configuration = configuration;
            _logger = logger;

            PushNotificationData["action"] = "TP_APPROVED";
        }
        ...
        protected override async Task SendPushNotification()
        {
            ...
            foreach (var item in EmailTextbookApproval.Textbooks)
            {
                ...
                _notificationData["Approver"] = item.NamaApprover;
        ...
        protected override async Task SaveNotification(IEnumerable<string> idUserRecipients, bool isBlast)
        {
            ...
            {
                _notificationData["Link"] = $"{_notificationData["UrlBase"]}?Id={item.Id}";
                _notificationData["Approver2"] = item.NamaApprover;
                PushNotificationData["id"] = item.Id;
                ...
                CreateNotificationHistory(
                    new List<string> { item.IdUserApprover },
                    isBlast,
        ...
        protected override async Task SendEmailNotification()
        {
            ...
            {
                _notificationData["Data"] = EmailTextbookApproval.Textbooks.Where(e => e.IdUserApprover == ItemUser.Id).ToList();
                _notificationData["Approver2"] = ItemUser.EmailAddress.Name;
                _notificationData["Subject"] = EmailTextbookApproval.Textbooks.Select(e=>e.Subject).FirstOrDefault();
                _notificationData["Grade"] = EmailTextbookApproval.Textbooks.Select(e=>e.Grade).FirstOrDefault();
                _notificationData["Title"] = EmailTextbookApproval.Textbooks.Select(e=>e.Title).FirstOrDefault();
                _notificationData["Author"] = EmailTextbookApproval.Textbooks.Select(e=>e.Author).FirstOrDefault();
