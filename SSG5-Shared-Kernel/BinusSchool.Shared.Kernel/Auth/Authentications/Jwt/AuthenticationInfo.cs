        ...
        public AuthenticationInfo(HttpRequest request, bool useLocalizeMessage = true)
        {
            ...
            catch (Exception ex)
            {
                Message = ex switch
                {
                    ...
                    SignatureVerificationException _ => useLocalizeMessage ? "ExTokenInvalidSignature" : "Token has invalid signature." ,
                    _ => ex.Message
                };
            ...
            if (claims.TryGetValue("role", out var roles) && roles != null)
                _roles = new Lazy<IEnumerable<ClaimsRole>>(() => JsonConvert.DeserializeObject<IEnumerable<ClaimsRole>>(roles.ToString()));
            if (claims.TryGetValue("tenant", out var tenants) && tenants != null)
                _tenants = new Lazy<IEnumerable<ClaimsTenant>>(() => JsonConvert.DeserializeObject<IEnumerable<ClaimsTenant>>(tenants.ToString()));
