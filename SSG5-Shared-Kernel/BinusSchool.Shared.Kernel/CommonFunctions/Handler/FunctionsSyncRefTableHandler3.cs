...
{
    /// <summary>
    /// This abstract class handler 3 only use for fn attendance sync from scheduling
    /// </summary>
    /// <typeparam name="T"></typeparam>
    public abstract class FunctionsSyncRefTableHandler3<T> where T : DbContext, IAppDbContext
    {
        ...
        private readonly ILogger<FunctionsSyncRefTableHandler3<T>> _logger;

        protected FunctionsSyncRefTableHandler3(IServiceProvider services)
        {
            _services = services;
            _configuration = services.GetService<IConfiguration>();
            _dbContext = services.GetService<T>();
            _logger = services.GetService<ILogger<FunctionsSyncRefTableHandler3<T>>>();
        }

        protected FunctionsSyncRefTableHandler3(IServiceProvider services, string hubName) : this(services)
        {
            _hubName = hubName;
        }
        ...
        protected async Task Synchronize(string message, Guid invocationId, CancellationToken cancellationToken)
        {
            ...
            _logger.LogInformation("Sync data from scheduling");
