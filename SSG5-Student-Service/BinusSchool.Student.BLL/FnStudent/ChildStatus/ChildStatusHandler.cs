...
{
    public class ChildStatusHandler : FunctionsHttpSingleHandler
    {
        private readonly IStudentDbContext _dbContext;
        public ChildStatusHandler(IStudentDbContext studentDbContext)
        {
        ...
        protected override async Task<ApiErrorResult<object>> Handler()
        {
            ...
            var predicate = PredicateBuilder.Create<LtChildStatus>(x => true);
            if (!string.IsNullOrWhiteSpace(param.Search))
                predicate = predicate.And(x
                    => EF.Functions.Like(x.Id, $"%{param.Search}%")
                    || EF.Functions.Like(x.ChildStatusName, $"%{param.Search}%"));

            var query = _dbContext.Entity<LtChildStatus>()
                .Where(predicate)
