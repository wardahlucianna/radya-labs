...
{
    public class DistrictHandler : FunctionsHttpSingleHandler
    {
        private readonly IStudentDbContext _dbContext;
        public DistrictHandler(IStudentDbContext studentDbContext)
        {
        ...
        protected override async Task<ApiErrorResult<object>> Handler()
        {
            ...
            var predicate = PredicateBuilder.Create<LtDistrict>(x => true);
            if (!string.IsNullOrWhiteSpace(param.Search))
                predicate = predicate.And(x
                    => EF.Functions.Like(x.Id, $"%{param.Search}%")
                    || EF.Functions.Like(x.DistrictName, $"%{param.Search}%"));

            var query = _dbContext.Entity<LtDistrict>()
                .Where(predicate)
