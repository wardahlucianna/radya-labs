...
{
    public class GetListStudentByAySmtLvlGrdHrmForExemplaryHandler : FunctionsHttpSingleHandler
    {
        ...
        public GetListStudentByAySmtLvlGrdHrmForExemplaryHandler(
        ...
        {
            var param = Request.ValidateParams<GetListStudentByAySmtLvlGrdHrmForExemplaryRequest>(
                    nameof(GetListStudentByAySmtLvlGrdHrmForExemplaryRequest.IdAcademicYear),
                    nameof(GetListStudentByAySmtLvlGrdHrmForExemplaryRequest.Semester)
                    );
            ...
            var getBlockedIdStudentList = await _dbContext.Entity<MsExemplaryStudentBlock>()
                                        .Where(x => _dateTime.ServerTime.Date >= x.BlockingStartDate.Date)
                                        .Select(y => y.IdStudent)
            ...
            else
            {
                var results = await query
                    ...
                    //.If(string.IsNullOrWhiteSpace(param.Search), x => x.SetPagination(param))
                    .Select(x => new GetListStudentByAySmtLvlGrdHrmForExemplaryResult
                    {
                        Id = x.Key.IdStudent,
                        ...
                        IsBlocked = getBlockedIdStudentList.Any(y => y == x.Key.IdStudent)
                    })
                    .ToListAsync(CancellationToken);

                var datas = new List<GetListStudentByAySmtLvlGrdHrmForExemplaryResult>();
