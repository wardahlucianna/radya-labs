...
{
    public class MedicineCategoryHandler : FunctionsHttpCrudHandler
    {
        private readonly IStudentDbContext _dbContext;
        public MedicineCategoryHandler(IStudentDbContext dbContext)
        {
        ...
        protected override async Task<ApiErrorResult<IReadOnlyList<IItemValueVm>>> GetHandler()
        {
            ...
            var columns = new[] { "medicinecategoryname" };

            var predicate = PredicateBuilder.Create<LtMedicineCategory>(x => true);
            if (!string.IsNullOrWhiteSpace(param.Search))
                predicate = predicate.And(x
                    => EF.Functions.Like(x.MedicineCategoryName, param.SearchPattern()));

            var query = _dbContext.Entity<LtMedicineCategory>()
                .Where(predicate)
            ...
            if (param.Return == CollectionType.Lov)
                items = await query
                    .Select(x => new ItemValueVm(x.Id, x.MedicineCategoryName))
                ...
                items = await query
                    ...
                    {
                        Id = x.Id,
                        Description = x.MedicineCategoryName
                    })
                    .ToListAsync(CancellationToken);
