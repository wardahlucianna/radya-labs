...
{
    public class MedicineTypeHandler : FunctionsHttpCrudHandler
    {
        private readonly IStudentDbContext _dbContext;
        public MedicineTypeHandler(IStudentDbContext dbContext)
        {
        ...
        protected override async Task<ApiErrorResult<IReadOnlyList<IItemValueVm>>> GetHandler()
        {
            ...
            var columns = new[] { "medicinetypename" };

            var predicate = PredicateBuilder.Create<LtMedicineType>(x => true);
            if (!string.IsNullOrWhiteSpace(param.Search))
                predicate = predicate.And(x
                    => EF.Functions.Like(x.MedicineTypeName, param.SearchPattern()));

            var query = _dbContext.Entity<LtMedicineType>()
                .Where(predicate)
            ...
            if (param.Return == CollectionType.Lov)
                items = await query
                    .Select(x => new ItemValueVm(x.Id, x.MedicineTypeName))
                ...
                items = await query
                    ...
                    {
                        Id = x.Id,
                        Description = x.MedicineTypeName
                    })
                    .ToListAsync(CancellationToken);
