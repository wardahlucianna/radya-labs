...
{
    public class GetClinicDailyReportExcludeInjuryVisitHandler : FunctionsHttpSingleHandler
    {
        private readonly IStudentDbContext _context;
        private readonly IPeriod _period;

        public GetClinicDailyReportExcludeInjuryVisitHandler(IStudentDbContext context, IPeriod period)
        {
            _context = context;
            _period = period;
        }
        ...
        protected override async Task<ApiErrorResult<object>> Handler()
        {
            ...
            var response = await GetClinicDailyReportExcludeInjuryVisit(request);

            return Request.CreateApiResult2(response as object);
        }

        public async Task<GetClinicDailyReportExcludeInjuryVisitReponse> GetClinicDailyReportExcludeInjuryVisit(ClinicDailyReportDataRequest request)
        {
            var incident = new List<GetClinicDailyReportExcludeInjuryVisitReponse_Incident>();
            ...
            #region Visit Data
            var injuryVisitIds = _context.Entity<TrMedicalRecordConditionDetails>()
                ...
                .Select(a => a.MedicalRecordEntry.Id)
                .Distinct()
                .ToList();

            var getConditionMedicalRecordVisitExcludeInjury = _context.Entity<TrMedicalRecordConditionDetails>()
                .Include(a => a.MedicalCondition)
                .Include(a => a.MedicalRecordEntry)
                .Where(a => a.MedicalCondition.IdSchool == request.IdSchool
                    && a.MedicalRecordEntry.IdSchool == request.IdSchool
                    && a.MedicalRecordEntry.CheckInDateTime.Date == request.Date.Date
                    && !a.MedicalCondition.MedicalConditionName.ToLower().Contains("injury")
                    && !injuryVisitIds.Contains(a.MedicalRecordEntry.Id))
                .GroupBy(a => new
                {
            ...
            int studentVisit = _context.Entity<MsStudent>()
                .AsEnumerable()
                .Where(a => getConditionMedicalRecordVisitExcludeInjury.Any(b => b.IdUser == a.Id))
                .Sum(a => getConditionMedicalRecordVisitExcludeInjury
                    .Where(b => b.IdUser == a.Id)
                    .Sum(b => b.VisitCount));

            int staffVisit = getStaff
                .AsEnumerable()
                .Where(a => getConditionMedicalRecordVisitExcludeInjury.Any(b => b.IdUser == a.IdBinusian))
                .Sum(a => getConditionMedicalRecordVisitExcludeInjury
            ...
            #region Student Data
            var studentData = from hs in getHomeroomStudent
                           join mri in getConditionMedicalRecordVisitExcludeInjury on hs.IdStudent equals mri.IdUser
                           select new
                           {
                               hs = hs,
                               mri = mri
            ...
            var insertStudent = studentData
                .Select(a => new GetClinicDailyReportExcludeInjuryVisitReponse_Incident
                {
                    Time = new GetClinicDailyReportExcludeInjuryVisitReponse_Incident.GetClinicDailyReportExcludeInjuryVisitReponse_Incident_Time
                    {
            ...
            var staffData = from s in getStaff
                            join mri in getConditionMedicalRecordVisitExcludeInjury on s.IdBinusian equals mri.IdUser
                            select new
                            {
                                s = s,
                                mri = mri,
            ...
            var otherPatientData = from op in getOtherPatient
                            join mri in getConditionMedicalRecordVisitExcludeInjury on op.Id equals mri.IdUser
                            select new
                            {
                                op = op,
                                mri = mri,
            ...
            var response = new GetClinicDailyReportExcludeInjuryVisitReponse()
            {
                Summary = new GetClinicDailyReportExcludeInjuryVisitReponse_Visit
                {
