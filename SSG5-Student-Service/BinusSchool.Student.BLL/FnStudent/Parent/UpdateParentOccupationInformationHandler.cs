...
{
    public class UpdateParentOccupationInformationHandler : FunctionsHttpSingleHandler
    {
        ...
        public UpdateParentOccupationInformationHandler(IStudentDbContext dbContext, IConfiguration configuration
        ...
        protected override async Task<ApiErrorResult<object>> Handler()
        {
            try
            {
                var body = await Request.ValidateBody<UpdateParentOccupationInformationRequest, UpdateParentOccupationInformationValidator>();

                var newParentInfoUpdateIdList = new List<string>();

                var newBody = new SetParentOccupationInformation
                {
                    IdOccupationType = new ItemValueVm { 
                        Id = body.IdOccupationType ?? "",
                        Description = body.IdOccupationTypeDesc ?? ""
                    },
                    IdParentSalaryGroup = new ItemValueVm
                    {
                        Id = body.IdParentSalaryGroup.ToString(),
                        Description = body.IdParentSalaryGroupDesc ?? ""
                    },
                    OccupationPosition = body.OccupationPosition,
