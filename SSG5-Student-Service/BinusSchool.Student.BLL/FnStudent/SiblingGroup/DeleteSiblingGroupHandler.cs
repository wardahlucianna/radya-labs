...
{
    public class DeleteSiblingGroupHandler : FunctionsHttpSingleHandler
    {
        ...
        public DeleteSiblingGroupHandler(IStudentDbContext dbContext, IConfiguration configuration, IMachineDateTime dateTime, IStudentEmailNotification sendEmailForProfileUpdateService)
        {
        ...
        protected override async Task<ApiErrorResult<object>> Handler()
        {
            try
            {
                ...
                var getdata = await _dbContext.Entity<MsSiblingGroup>().Where(x => x.Id == body.IdSiblingGroup && x.IdStudent == body.IdSibling).FirstOrDefaultAsync();
                if (getdata is null)
                ...
                var newVal = System.Guid.NewGuid();
