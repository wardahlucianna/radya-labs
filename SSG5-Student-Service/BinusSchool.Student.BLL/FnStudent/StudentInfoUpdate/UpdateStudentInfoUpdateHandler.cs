...
{
    public class UpdateStudentInfoUpdateHandler : FunctionsHttpSingleHandler
    {
        ...
        public UpdateStudentInfoUpdateHandler(IStudentDbContext dbContext, IConfiguration configuration
        ...
        protected override async Task<ApiErrorResult<object>> Handler()
        {
            try
            {
                ...
                {
                    var getdata = await _dbContext.Entity<MsStudent>().Where(p => p.Id == body.IdUser).FirstOrDefaultAsync();
                    if (getdata is null)
                    ...
                    if (body.IdApprovalStatus == 1)
                    {
                        ...
                        // Retrieve the maximum length of the direct column
                        var maxLength = ((DbContext)_dbContext).Model.FindEntityType(typeof(MsStudent))
                              .FindProperty(body.FieldName).GetMaxLength();

                        if (maxLength.HasValue &&
                            convertedObject is string strValue &&
                            strValue.Length > maxLength.Value)
                        {
                            // The value exceeds the maximum length of the column
                            throw new ArgumentException($"The length of the {body.FieldName} field cannot exceed {maxLength.Value} characters.");
                        }
                        else
                        {
                            // The value is within the maximum length of the column
                            propertyInfo.SetValue(getdata, convertedObject);
                            _dbContext.Entity<MsStudent>().Update(getdata);
                        }
                    }
                }
