...
namespace BinusSchool.Student.FnStudent.City
{
    ...
    {
        private const string _route = "student/city";
        private const string _tag = "Literal table";

        private readonly CityHandler _cityHandler;
        public CityEndPoint(CityHandler cityHandler)
        {
            _cityHandler = cityHandler;
        }

        [FunctionName(nameof(CityEndPoint.GetCities))]
        [OpenApiOperation(tags: _tag, Summary = "Get City List")]
        ...
        [OpenApiParameter(nameof(GetCityRequest.IdProvince), In = ParameterLocation.Query, Required = true )]
        [OpenApiParameter(nameof(GetCityRequest.IdCountry), In = ParameterLocation.Query, Required = true)]
        ...
        {
            return _cityHandler.Execute(req, cancellationToken);
        }
    }
}
