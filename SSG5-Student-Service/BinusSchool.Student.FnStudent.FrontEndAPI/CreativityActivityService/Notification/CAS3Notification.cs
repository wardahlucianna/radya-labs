...
{
    public class CAS3Notification : FunctionsNotificationHandler
    {
        private IDictionary<string, object> _notificationData;
        private readonly ILogger<CAS3Notification> _logger;
        private readonly IStudentDbContext _dbContext;
        private readonly IConfiguration _configuration;

        public CAS3Notification(INotificationManager notificationManager, IConfiguration configuration, ILogger<CAS3Notification> logger, IStudentDbContext dbContext, IDictionary<string, object> notificationData) :
           base("CAS3", notificationManager, configuration, logger)
        {
        ...
        protected override async Task SendEmailNotification()
        {
            ...
            foreach(var IdUser in GetExperienceEmail.IdUserCASList)
            {
                var GetUserById = User.Where(e => e.Id == IdUser).FirstOrDefault();
                if (GetUserById == null)
                    continue;

                _notificationData["CasAdvisorName"] = GetUserById.EmailAddress.Name;
                _notificationData["StudentName"] = GetExperienceEmail.StudentName;
                _notificationData["BinusianId"] = GetExperienceEmail.BinusianId;
