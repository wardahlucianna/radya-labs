...
namespace BinusSchool.Student.FnStudent.District
{
    ...
    {
        private const string _route = "student/district";
        private const string _tag = "Literal table";

        private readonly DistrictHandler _districtHandler;
        public DistrictEndPoint(DistrictHandler districtHandler)
        {
            _districtHandler = districtHandler;
        }

        [FunctionName(nameof(DistrictEndPoint.GetDistricts))]
        [OpenApiOperation(tags: _tag, Summary = "Get District List")]
        ...
        public Task<IActionResult> GetDistricts(
           [HttpTrigger(AuthorizationLevel.Function, "get", Route = _route)] HttpRequest req,
           CancellationToken cancellationToken)
        {
            return _districtHandler.Execute(req, cancellationToken);
        }
    }
}
