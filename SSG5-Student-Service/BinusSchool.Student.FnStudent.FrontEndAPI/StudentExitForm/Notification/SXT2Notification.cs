...
{
    public class SXT2Notification : FunctionsNotificationHandler
    {
        ...
        private readonly ILogger<SXT2Notification> _logger;

        public SXT2Notification(INotificationManager notificationManager, IConfiguration configuration, ILogger<SXT2Notification> logger, IStudentDbContext dbContext, IDictionary<string, object> notificationData) :
            base("SXT2", notificationManager, configuration, logger)
        {
        ...
        protected override async Task SendEmailNotification()
        {
            try
            {
                ...
                _notificationData = new Dictionary<string, object>
                    {
                        { "Acop", _userName.TrimStart().TrimEnd() },
        ...
        protected override async Task SaveNotification(IEnumerable<string> idUserRecipients, bool isBlast)
        {
            try
            {
                ...
                foreach (var idUser in IdUserRecipients)
                {
                    ...
                    {
                        GeneratedContent = $"{_studentExitResult.ParentName} has cancelled the student exit submission. Please check on Student Exit Submission Menu";
                    }
                    else
                    {
