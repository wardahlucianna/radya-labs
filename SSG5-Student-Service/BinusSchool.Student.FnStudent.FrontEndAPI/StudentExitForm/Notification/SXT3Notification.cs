...
{
    public class SXT3Notification : FunctionsNotificationHandler
    {
        ...
        private readonly ILogger<SXT3Notification> _logger;

        public SXT3Notification(INotificationManager notificationManager, IConfiguration configuration, ILogger<SXT3Notification> logger, IStudentDbContext dbContext, IDictionary<string, object> notificationData) :
            base("SXT3", notificationManager, configuration, logger)
        {
        ...
        protected override async Task SaveNotification(IEnumerable<string> idUserRecipients, bool isBlast)
        {
            try
            {
                ...
                foreach (var idUser in IdUserRecipients)
                {
                    ...
                    {
                        GeneratedContent = $"Binus School {_schoolName} has approved with note your request for the student exit submission. Please check on Student Exit page.";
                    }
                    else
                    {
