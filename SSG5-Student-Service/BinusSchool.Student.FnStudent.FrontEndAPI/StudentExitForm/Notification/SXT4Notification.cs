...
{
    public class SXT4Notification : FunctionsNotificationHandler
    {
        ...
        private readonly ILogger<SXT4Notification> _logger;

        public SXT4Notification(INotificationManager notificationManager, IConfiguration configuration, ILogger<SXT4Notification> logger, IStudentDbContext dbContext, IDictionary<string, object> notificationData) :
            base("SXT4", notificationManager, configuration, logger)
        {
        ...
        protected override async Task SaveNotification(IEnumerable<string> idUserRecipients, bool isBlast)
        {
            try
            {
                ...
                foreach (var idUser in IdUserRecipients)
                {
                    ...
                    {
                        GeneratedContent = $"Binus School {_schoolName} has approved your request for the student exit submission. Please check on Student Exit page.";
                    }
                    else
                    {
