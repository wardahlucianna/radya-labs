...
{
    public class MessageAddMessageHandler : FunctionsHttpSingleHandler
    {
        ...
        public MessageAddMessageHandler(IUserDbContext dbContext, IConfiguration configuration, IRolePosition apiRolePosition, IMessage apiMessage)
        {
        ...
        protected override async Task<ApiErrorResult<object>> Handler()
        {
            ...
            var message = _dbContext.Entity<TrMessage>()
                ...
                .Include(X => X.MessageFors).ThenInclude(e=>e.MessageForDepartements)
                .Include(X => X.MessageFors).ThenInclude(e=>e.MessageForGrades)
                .Include(X => X.MessageFors).ThenInclude(e=>e.MessageForPositions)
            ...
            trMessage.Id = message==null ? Guid.NewGuid().ToString(): message.Id;
            ...
            else if (body.Type == UserMessageType.Announcement || body.Type == UserMessageType.Information)
            {
                ...
                if (body.Type == UserMessageType.Information && !body.GroupMembers.Any())
                {
                    ...
                    if (body.MessageFor.Any())
                    {
                        ...
                        paramGetUserRolePosition.UserRolePositions= listUserRolePosition;
            ...
            var groupMembers = body.GroupMembers;
            ...
            if (message != null)
            {
                ...
                var existingRecepients = await _dbContext.Entity<TrMessageRecepient>().Where(x => x.IdMessage == trMessage.Id && x.MessageFolder == MessageFolder.Inbox).Select(x => x.IdRecepient).ToListAsync(CancellationToken);
                var existingAttachments = await _dbContext.Entity<TrMessageAttachment>().Where(x => x.IdMessage == trMessage.Id).ToListAsync(CancellationToken);

                var deleteRecepients = existingRecepients.Where(x => !body.Recepients.Contains(x)).ToList();
                ...
                if (deleteAttachments.Count > 0)
                {
                    ...
                    {
                        var attachment = trMessage.MessageAttachments.Where(x => x.Id == a.Id).FirstOrDefault();
                        if (attachment == null)
                ...
                recepients = body.Recepients.Where(x => !existingRecepients.Contains(x)).ToList();
            ...
            var listRecepient = recepients
                                .Where(e => !listIdRecepient.Contains(e))
                                .ToList();

            if (body.MessageFor.Any() || body.GroupMembers.Any())
            {
                listRecepient = recepients
                                .ToList();
            }

            if (listRecepient.Any())
            {
                var addRecepient = await _dbContext.Entity<MsUser>()
                                .Where(e => listRecepient.Contains(e.Id))
            ...
            if (groupMembers != null)
                if (groupMembers.Any())
                {
                    _dbContext.Entity<TrMessageGroupMember>().AddRange(groupMembers.Select(x => new TrMessageGroupMember
                    {
                        Id = Guid.NewGuid().ToString(),
                        IdMessage = trMessage.Id,
                        IdGroupMailingList = x
                    }));
                }
        ...
        public async Task SendEmailNotif(TrMessage trMessage, List<string> recepients)
        {
            ...
            paramTemplateEmailNotification.Add("idRecipients", string.Join(",", recepients));
        ...
        public async Task SentNotifToRecipient(TrMessage trMessage, string idUser)
        {
            ...
            //templateNotif.Add("recipient", messageRecepient.Select(x => x.IdRecepient));
            templateNotif.Add("subject", subjectMessage);
            templateNotif.Add("sendBy", sendByMessage);

            if (messageRecepient.Select(x => x.IdRecepient).Count() > 1000)
            {
