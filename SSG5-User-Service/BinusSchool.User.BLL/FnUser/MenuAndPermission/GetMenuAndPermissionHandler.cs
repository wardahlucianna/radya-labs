...
{
    public class GetMenuAndPermissionHandler : FunctionsHttpSingleHandler
    {
        private readonly IUserDbContext _dbContext;

        public GetMenuAndPermissionHandler(IUserDbContext dbContext)
        {
        ...
        protected async override Task<ApiErrorResult<object>> Handler()
        {
            var features = await _dbContext.Entity<MsFeature>()
                                           ...
                                           .Where(x => x.IsShowMobile == false)
                                           .ToListAsync();

            if (features.Any())
            {
                var webFeatures = features.Where(x => string.IsNullOrEmpty(x.IdParent)).OrderBy(x => x.OrderNumber).Select(x => new MenuAndPermissionResult
                {
                    Id = x.Id,
                    Code = x.Code,
                ...
                return Request.CreateApiResult2(webFeatures as object);
            }
