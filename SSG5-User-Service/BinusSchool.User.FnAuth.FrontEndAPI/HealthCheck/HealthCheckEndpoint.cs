...
namespace BinusSchool.User.FnAuth.HealthCheck
{
    public class HealthCheckEndpoint : FunctionsHealthCheckHandler
    {
        ...
        [FunctionName(nameof(GetEnvironments))]
        [OpenApiOperation(tags: Tag)]
        [OpenApiResponseWithoutBody(System.Net.HttpStatusCode.OK)]
        public IActionResult GetEnvironments(
            [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = Route + "/env")] HttpRequest req,
            ILogger log,
            CancellationToken cancellationToken)
        {
