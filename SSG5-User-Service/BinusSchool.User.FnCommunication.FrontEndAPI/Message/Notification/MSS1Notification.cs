...
{
    public class MSS1Notification : FunctionsNotificationHandler
    {
        ...
        private List<string> _recipientNames;

        public MSS1Notification(string idScenario, INotificationManager notificationManager, IConfiguration configuration,
            ILogger<FunctionsNotificationHandler> logger, IUserDbContext dbContext, IDictionary<string, object> notificationData) : base("MSS1", notificationManager, configuration, logger)
        {
        ...
        protected async override Task Prepare()
        {
            ...
            var idRecipients = _notificationData.Where(x => x.Key == "idRecipients").Select(x => x.Value).FirstOrDefault().ToString();

            if (idRecipients != null)
            {
                var splitIdRecipients = idRecipients.Split(",");
                _recipientNames = await _dbContext.Entity<MsUser>().Where(x => idRecipients.Contains(x.Id)).Select(x => x.DisplayName).ToListAsync();
                _notificationData.Add("recipient", string.Join(",", _recipientNames));
            }
            else
            {
                _notificationData.Add("recipient", null);
            }

            _notificationData.Remove("idRecipients");
