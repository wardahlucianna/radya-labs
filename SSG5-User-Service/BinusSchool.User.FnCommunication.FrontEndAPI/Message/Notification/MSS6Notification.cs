...
{
    public class MSS6Notification : FunctionsNotificationHandler
    {
        ...
        public MSS6Notification(string idScenario, INotificationManager notificationManager, IConfiguration configuration,
            ILogger<FunctionsNotificationHandler> logger, IUserDbContext dbContext, IDictionary<string, object> notificationData) : base("MSS6", notificationManager, configuration, logger)
        {
        ...
        protected async override Task Prepare()
        {
            ...
            _idMessage = KeyValues.Where(x => x.Key == "id").Select(x => x.Value).FirstOrDefault().ToString();

            _messageType = KeyValues.Where(x => x.Key == "messageType").Select(x => x.Value).FirstOrDefault().ToString();

            var notes = await _dbContext.Entity<TrMessageApproval>().Where(x => x.IdMessage == _idMessage)
                .OrderByDescending(x => x.DateIn)
                .Select(x => x.Reason)
                .FirstOrDefaultAsync(CancellationToken);

            var url = $"{_configuration.GetSection("ClientApp:Web:Host").Get<string>()}/communication/messagesent?id={_idMessage}&mgType={_messageType}";
        ...
        protected async override Task SendPushNotification()
        {
            ...
            //data push notif
            PushNotificationData["action"] = "MS_SENT";
